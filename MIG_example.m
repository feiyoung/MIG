...
tic; 
[hX, hD, hHm, hBm,cvVals, history] = MIG(Xmis, group, type, q, 1e-6, 10, lambda, verbose, 0);
time = toc
...
% Xmis(:,group~=3) = Xmis(:,group~=3) *10000;
% [Xmiss, group1, type1, type_scale] = precheckdata(Xmis, group, type);
% Xmis(:,end) = 0;
% q= 3;
% verbose = 1; lambda =10;
% % imputation using MIG
% tic; 
% [hX, hD, hHm, hBm,cvVals, history] = MIG(Xmis, group, type, q, 1e-6, 2, lambda, verbose, 0);
% time = toc
% [Xmis(1:10, 1), hX(1:10,1), X(1:10,1)]
% [Xmis(1:10, 69), hX(1:10,69), X(1:10,69)]
